...
public class Book {
    ...
    public static String capitalize(String word) {
		word=word.trim();   	
		word=word.isEmpty()?"":word.substring(0, 1).toUpperCase()+word.substring(1).toLowerCase();
    	return word;
    }
    ...
    public static boolean isTitleWord(String word) {
    	...
    	{
    		 if (a.equalsIgnoreCase(word)) return false;
    	}
    ...
    public void setAuthor(String author) {
    	...
    	if (!author.isEmpty())
    	{
        	...
        	for(String a:authorFullName)     
        	{
        		this.author+=capitalize(a)+" ";
        	}
     ...
     *
     */
    public void setTitle(String title) {System.out.println(title);
    	
    if(!title.isEmpty())
    	{	this.title="";
    		String[] fullTitle=title.split(" ");
    		int wordCount=0;
    		for(String word:fullTitle)
    		{       	
    			if(isTitleWord(word) || wordCount==0) 
    				this.title+=capitalize(word)+" ";
    			else 
    				this.title+=word.toLowerCase()+" ";
